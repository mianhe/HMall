...
public class CatalogTestContext {
    ...
    /** 通过 optionValue 查找 optionId（遍历所有维度） */
    public Long findOptionIdByValue(String productName, String optionValue) {
        for (var entry : optionIdByProductDimensionAndValue.entrySet()) {
            if (entry.getKey().startsWith(productName + ":") && entry.getKey().endsWith(":" + optionValue)) {
    ...
    /** 通过 optionValue 查找所属 dimensionId */
    public Long findDimensionIdByOptionValue(String productName, String optionValue) {
        for (var entry : optionIdByProductDimensionAndValue.entrySet()) {
            if (entry.getKey().startsWith(productName + ":") && entry.getKey().endsWith(":" + optionValue)) {
                // key 格式: productName:dimensionName:optionValue
    ...
    /** 获取该商品的第一个维度 ID */
    public Long getFirstDimensionId(String productName) {
        for (var entry : dimensionIdByProductAndName.entrySet()) {
            if (entry.getKey().startsWith(productName + ":")) {
                return entry.getValue();
            }
        }
        return null;
    ...
    /** 获取该商品的第一个选项 ID（任意维度下） */
    public Long getFirstOptionId(String productName) {
        for (var entry : optionIdByProductDimensionAndValue.entrySet()) {
            if (entry.getKey().startsWith(productName + ":")) {
                return entry.getValue();
            }
