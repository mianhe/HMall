...
功能: 库存管理（管理后台）
  ...
  场景: 按 skuId 列表批量查询库存
    假如 skuId 3001 可用库存为 15
    并且 skuId 3002 可用库存为 25
    当 管理端 批量查询 skuId 列表 3001、3002、9999 的库存
    那么 应返回 200
    并且 返回的库存列表应包含 skuId 3001 且 available 15
    并且 返回的库存列表应包含 skuId 3002 且 available 25
    并且 返回的库存列表长度应为 2

  场景: 查询不存在的 skuId 时应返回 404
    当 管理端 查询 skuId 9999 的库存
    那么 应返回 404

  场景: 设置 available 为负数时应返回 400
    当 管理端 对 skuId 1001 设置可用库存为 -1
    那么 应返回 400
