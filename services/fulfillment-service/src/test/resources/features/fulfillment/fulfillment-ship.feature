...
@fulfillment
功能: 发货
  作为管理后台操作人员，我希望对 CREATED 状态的履约单执行发货，以便推进履约流程。

  场景: CREATED 状态的履约单执行发货应成功且状态变为 SHIPPED
  ...
  场景: 非 CREATED 状态执行发货应失败并返回错误
    假如 已存在 SHIPPED 状态的履约单 orderId 9003
    当 对该履约单执行发货 承运商 "顺丰" 物流单号 "SF1234567892"
    那么 应返回 400

  场景: 履约单不存在时执行发货应返回 404
    当 对不存在的履约单 ID 999999 执行发货 承运商 "顺丰" 物流单号 "SF1234567893"
    那么 应返回 404
