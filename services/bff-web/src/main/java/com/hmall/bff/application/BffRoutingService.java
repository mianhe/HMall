...
public class BffRoutingService {
    ...
    private Downstream resolve(String path) {
        if (path.startsWith("/api/categories") || path.startsWith("/api/products") || path.startsWith("/api/skus") || path.startsWith("/api/files") || path.startsWith("/api/engraving-patterns")) {
