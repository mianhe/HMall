<template>
  ...
  <Transition name="slide">
    <div
      v-if="open"
      class="fixed top-0 right-0 bottom-0 w-[400px] max-w-full z-50 flex flex-col bg-gray-50 shadow-2xl"
    >
      <!-- Header -->
      <div class="flex items-center justify-between px-4 py-3 bg-white border-b border-gray-200">
        <div class="flex items-center gap-2">
          <h2 class="font-semibold text-gray-800">智能助手</h2>
          <select
      ...
      <Transition name="fade">
        ...
        >
          ...
          <span>{{ chat.lastMatchedSkills.value.map(s => s.name).join('、') }}</span>
        </div>
      </Transition>
