...
> 用于按 FSD 与 Atomic Design 实现前端的输入文档。  
> **与当前实现的关系**：当前管理端仅实现「首页 + Catalog 树形只读」（见 `requirements.md`）。本文档描述的是**类目/商品 CRUD、商品详情等扩展设想**，尚未实现；后续若做表单页、列表页可据此实现。  
> **你无需事先填这份文档**——需要你做选择时，我会用简单问题引导你，并帮你把答案写进这里。
...
| `/fulfillment` | 履约管理：表格（履约单ID、订单ID、状态、商品摘要、收货人、地址、承运商、物流单号、发货/签收时间、创建时间、操作），过滤（订单ID、状态），按状态操作（开始配货、发货、签收）（已实现） |
