...
- ✅ 2.12 按类目查询商品列表时，支持 `include=skus` 参数；携带该参数时，每个商品对象内嵌其 SKU 列表（含 specValues 和价格）；不携带时保持原有行为
- ✅ 2.13 按关键词搜索商品时，同样支持 `include=skus` 参数，行为与 2.12 一致
...
## 6. 管理镭雕图案库  
`engraving-pattern.feature`

> 以下变更来自业务需求 [镭雕服务](../../business-requirements/laser-engraving/overview.md)

镭雕服务的可选图案，供用户下单时选择。Admin 管理，前端选品时通过 `GET /api/engraving-patterns` 查询。

- ✅ 6.1 创建镭雕图案时应成功并返回图案信息（name、imageUrl 必填；sortOrder、enabled 可选，enabled 默认 true）
- ✅ 6.2 修改已有镭雕图案时应成功并返回新信息
- ✅ 6.3 删除已有镭雕图案后该图案不再存在
- ✅ 6.4 查询镭雕图案列表时应返回按 sortOrder 排序的图案；支持 enabled 过滤（不传则返回全部，enabled=true 仅返回启用的）
- ✅ 6.5 按 ID 查询镭雕图案详情时应返回图案信息
...
本节定义 Catalog BC 对外的性能承诺。所有指标基于单实例部署、PostgreSQL 16、数据量 ≤ 500 SPU / 5000 SKU 的前提。

### 7.1 响应时间（P95，服务端处理时间，不含网络传输）

| 操作类型 | 端点示例 | P95 目标 |
|---------|---------|---------|
| 单实体读取 | `GET /products/{id}`、`GET /skus/{id}` | ≤ 200ms |
| 列表查询 | `GET /products?categoryId=`、`GET /products/search` | ≤ 500ms |
| 列表查询（含 SKU） | `GET /products?categoryId=&include=skus` | ≤ 1s |
| 完整类目树 | `GET /categories/tree` | ≤ 300ms |
| 写操作 | `POST/PUT/DELETE` 各资源 | ≤ 500ms |

### 7.2 设计约束

- **无级联删除**：删除类目时若存在子类目或商品应拒绝，不做级联清理。
- **SKU 内嵌按需加载**：`include=skus` 仅在调用方显式请求时才执行 SKU 查询，默认不加载，避免不必要的性能开销。
- **索引覆盖**：所有外键列和常用查询列（`category_id`、`spu_id`、`spec_dimension_id`、`spec_option_id`）须有数据库索引。
...
| 类目总层级 | 3 | 5 |
| SPU 总数 | 28 | 500 |
| 单 SPU 下 SKU | 最多 37 | 100 |
| SKU 总数 | 146 | 5000 |

超出设计上限时需重新评估 SLA 指标和索引策略。
...
| 2. 管理商品 | product.feature | 2.1～2.13 | ✅ 全部已实现（含 include=skus） |
...
| 6. 管理镭雕图案库 | engraving-pattern.feature | 6.1～6.6 | ✅ 已实现 |
