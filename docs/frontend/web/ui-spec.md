...
- **增值服务（保障/镭雕）**：实体商品（PHYSICAL）详情页在规格选择区下方展示服务选项按钮；默认不选，点击选中，再次点击同项取消。可选服务通过 `GET /api/products/{spuId}/available-services` 获取。
- **镭雕内容**：当选中的服务为镭雕（`serviceKind=ENGRAVING`）时，展示图案选择（`GET /api/engraving-patterns?enabled=true`）+ 文字输入（≤20 字）；至少选其一；校验通过后方可「立即购买」或「加入购物车」。加入购物车时镭雕内容不存，结账页填写（迭代 2）；立即购买时携带 `serviceAttributes` 至结账。
