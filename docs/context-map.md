...
| **Fulfillment** | 拆单、配货、发货、配送 | ✅ 已实现 | 端口 8088；6 feature、24 scenario 全绿；已与 Order 集成（同步创建/取消 + Kafka Allocated/Shipped/Delivered）；Kafka 事件发布已实现 |
| **Activity** | 消费各 BC 事件，构建业务活动记录（审计、统计、监控） | ✅ 已实现 | 订阅 Order/Payment/Inventory/Fulfillment Kafka 事件；3 feature、16 scenario |
