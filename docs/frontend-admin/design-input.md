...
| `/activity` | 活动监控：统计仪表盘 + 最近事件流水 |
| `/activity/journey/:orderId?` | 订单旅程回放：按 BC 分组的事件时间线，展示交易全生命周期与 Saga 补偿路径 |
| `/settings` | 系统设置：按分类展示配置项（当前含「支付设置」） |
...
| 展开/折叠 | 各级可展开或折叠 | 树形控件 |
| 刷新 | 重新拉取数据 | 刷新按钮 |
| 活动监控 | 统计卡片 + 时间范围切换 + 最近活动流水 | `/activity` |
...
| 活动监控 | AppHeader + 时间范围选择器（pill tabs + 日期输入）+ 统计卡片组（订单/支付/库存 分组标题 + grid 卡片）+ 最近活动表格 + 订单旅程入口（输入框 + 跳转按钮） |
| 订单旅程回放 | AppHeader + 面包屑（活动监控 > 订单旅程）+ 顶部概要卡片（orderId、推断状态、事件数、时间跨度）+ 分组垂直时间线（BC 彩色 badge + 中文事件名 + 时间戳 + 可展开 payload 详情；补偿事件红/橙色高亮） |
| 系统设置 | AppHeader + 分类卡片（支付设置：超时时间输入框 + 保存按钮） |
