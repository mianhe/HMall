...
### 步骤 3.5：代码整洁检查（Smoke 通过后）

自动化验证通过后，对本次变更的前端文件做一轮轻量整理。不改变行为，Smoke 仍须全绿。

| 关注点 | 动作 |
|--------|------|
| **组件体量** | 若 `<script setup>` 超过 ~300 行或模板超过 ~200 行，考虑提取子组件或 composable |
| **重复逻辑** | 页面间重复的 API 调用模式、数据处理逻辑可提取到 `shared/` 或 composable |
| **命名一致** | 变量名、函数名与后端 API 字段名 / 领域术语保持对齐 |
| **共享层整洁** | `shared/api/` 无废弃函数；`shared/ui/` 组件职责单一 |
| **模板简化** | 过深的嵌套提取为具名子组件或计算属性 |

> 若发现需要较大的结构调整（如整页重构），记录为遗留项，不在本步骤执行。
...
- [ ] **代码整洁检查已完成**：组件体量合理、无重复逻辑、命名一致、共享层整洁
